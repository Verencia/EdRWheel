...
function cancelSession (sessionCount) {
  ...
  loseTotal = localStorage.getItem("loseTotal");
  histoTirage = (JSON.parse(localStorage.getItem("histoTirage")));
  let historique = (JSON.parse(localStorage.getItem("histo")));
...
function archivedGames() {
  ...
  gameArchive.forEach(game => {
    ...
    document.getElementById("archivedGames").appendChild(div);
    //gamesArr.push(div);
    lilcount++;
  });
...
function deleteGame(id) {
  ...
  if (confirm("Voulez vous vraiment supprimer ce jeu ? (Vous pouvez le récupérer dans les archives)") && theWheel.segments.length > 1) {
    let deleted = false;
    let games = JSON.parse(window.localStorage.getItem("games"));
    console.log("avant");
    games.forEach(game => {
